...
//
// Purpose: Surrender behavior component which allows NPCs to be detained. Partially based on ai_behavior_fear. Ripped straight from EZ2's code
// 
...
CBaseCombatCharacter* CAI_SurrenderBehavior::GetNearestPotentialCaptor(bool bIncludingPlayer, bool bFirst, int* pNumCaptors, bool* bCanBeSeen)
{
	...
	for (int i = 0; i < nAIs; i++)
	{
		...
		if (pNPC->IsAlive())
		{
			// CLASS_CITIZEN_REBEL isn't considered, as we just use that as an enemy/non-Lambda Resistance rebel, i.e Rioting citizens in C17
			...
			// Unarmed citizens aren't a thread
...
CBaseCombatCharacter* CAI_SurrenderBehavior::GetNearestPotentialRescuer(bool bIncludingPlayer, bool bFirst, int* pNumRescuers)
{
	...
	for (int i = 0; i < nAIs; i++)
	{
		...
		if (pNPC->IsAlive())
		{
			// Only Overwatch soldiers and Metropolice/Combine Security will attempt rescue
			bool bCombine = false;
			Class_T classify = pNPC->Classify();
			switch (classify)
			{
...
void CAI_SurrenderBehavior::FindSurrenderDest(CAI_Hint** pOutHint, Vector& vecDest, Vector& vecDir)
{
	...
	for (int node = 0; node < g_pBigAINet->NumNodes(); node++)
	{
		...
		// Just gonna use HINT_PLAYER_ALLY_FEAR_DEST
		CAI_Hint* pHint = pNode->GetHint();
		if (pHint && pHint->HintType() == HINT_PLAYER_ALLY_FEAR_DEST && (pHint->GetGroup() == NULL_STRING || pHint->GetGroup() == GetOuter()->GetHintGroup()))
		{
